...
# Last Working no breakeven notice
#
# async def ws_user_msg(ws, msg):
...
#                 profit = float(o['rp'])
#                 trade.result += profit
#                 logging.info(f"Обновление прибыли для {symbol}: +{profit} => {trade.result}")
#
#             position_open = positions.get(symbol, False)
#
#             # --- ✅ Уведомление о частичном закрытии (Take1) ---
#             if (
...
#                         )
#                 except Exception as e:
#                     logging.error(f"{symbol}: ошибка при отправке уведомления о частичном закрытии: {e}")
#
#                 async with session() as s:
#                     await s.execute(
...
#                     await s.commit()
#
#             # --- ✅ Уведомление при полном закрытии (однократно) ---
...
#             logging.info(f"{symbol}: новые ордера и флаги успешно сохранены в БД.")
        ...
        # --- Уведомление в TG --- ЛУЧШЕ ОТПРАВЛЯТЬ ЧЕРЕЗ WS USER MSG
        ...
        #         )
        # except Exception as e:
        #     logging.error(f"{symbol}: ошибка при отправке уведомления о переносе стопа в БУ: {e}")
        #
...
async def partial_close_and_move_stop(trade):
    ...
    try:
        ...
        msgs.append(f"{symbol}: ошибка после частичного закрытия: {e}")
